...
<?php $mpmc_options = MpmcHelper::fetch_options(); ?>
    ...
    <div id="general" class="mepr-options-hidden-pane">
      ...
      <table class="form-table">
        <tbody>
          ...
          <tr valign="top">
            <th scope="row">
              <label for="<?php echo $mepr_options->currency_code_str; ?>"><?php _e('Base Currency Code:', 'memberpress'); ?></label>
            </th>
            <td>
          ...
          <tr valign="top">
            <th scope="row">
              <label for="<?php echo $mepr_options->currency_symbol_str; ?>"><?php _e('Base Currency Symbol:', 'memberpress'); ?></label>
            </th>
            <td>
              <?php MeprOptionsHelper::payment_currencies_dropdown($mepr_options->currency_symbol_str, $mepr_options->currency_symbol); ?>
            </td>
          </tr>
          <!-- MultiCurrency Starts -->

					<tr valign="top">
						<th scope="row">
						<label for="<?php echo $mepr_options->currency_code_str; ?>"><?php _e( 'Currencies:', 'memberpress' ); ?></label>
						</th>
						<td>
						<?php MpmcHelper::payment_multicurrencies_dropdown( $mepr_options->currency_code_str, $mpmc_options['mpmc_currencies'] ); ?>
						</td>
					</tr>
					<tr valign="top" id="mpmc-service-provider">
						<th scope="row">
						<label for="mpmc_service_provider"><?php _e( 'Service Provider:', 'memberpress' ); ?>
              <?php MeprAppHelper::info_tooltip( 'mepr-currency-symbol-after',
                                                 __('Exchange Service Provider', 'memberpress'),
                                                 __('<a href="https://openexchangerates.org/">OpenExchangeRate</a><br/>If using free account, base currency must be set to USD. To use any currency as a a base currency, choose either manual or <a href="https://www.exchangerate-api.com/">ExchangeRate API</a>', 'memberpress') . '</strong>' ); ?>            
            </label>
						</th>
						<td>
							<select name="mpmc_service_provider" id="">
							<option value="openx" <?php selected( $mpmc_options['mpmc_service_provider'], 'openx' ); ?>><?php _e( 'Open Exchange Rates', 'memberpress' ); ?></option>
							<option value="exrate" <?php selected( $mpmc_options['mpmc_service_provider'], 'exrate' ); ?>><?php _e( 'Exchange Rate API', 'memberpress' ); ?></option>
							<option value="manual" <?php selected( $mpmc_options['mpmc_service_provider'], 'manual' ); // Mapped as "Medium" ?>><?php _e( 'Manual', 'memberpress' ); ?></option>
							</select>
						</td>
					</tr>
					<tr valign="top" class="mepr-hidden" id="mpmc-openx-api">
						<th scope="row">
						<label for="mpmc_openx_api"><?php _e( 'Open Exchange Rates API', 'memberpress' ); ?></label>
						</th>
						<td>
						<input type="text" class="regular-text" name="mpmc_openx_api" value="<?php echo $mpmc_options['mpmc_openx_api']; ?>" />
						<?php
						if ( ! empty( $mpmc_options['mpmc_openx_timestamp'] ) ) {
							?>
							<div><span style="font-style:italic; font-size:0.8em"> <?php printf( '%s %s %s', esc_html( 'Rates updated on ', 'mpmc' ), date_i18n( 'F j, Y g:ia', $mpmc_options['mpmc_openx_timestamp'] ), '(UTC)', 'mpmc' ); ?></span></div>
						<?php } ?>
						</td>
					</tr>
          <tr valign="top" class="mepr-hidden" id="mpmc-exrate-api">
						<th scope="row">
						<label for="mpmc_exrate_api"><?php _e( 'Exchange Rate API', 'memberpress' ); ?></label>
						</th>
						<td>
						<input type="text" class="regular-text" name="mpmc_exrate_api" value="<?php echo $mpmc_options['mpmc_exrate_api']; ?>" />
						<?php
						if ( ! empty( $mpmc_options['mpmc_exrate_timestamp'] ) ) {
							?>
							<div><span style="font-style:italic; font-size:0.8em"> <?php printf( '%s %s %s', esc_html( 'Rates updated on ', 'mpmc' ), date_i18n( 'F j, Y g:ia', $mpmc_options['mpmc_exrate_timestamp'] ), '(UTC)', 'mpmc' ); ?></span></div>
						<?php } ?>
						</td>
					</tr>
					<tr valign="top" id="mpmc-switcher-position">
						<th scope="row">
						<label for="mpmc_switcher_position"><?php _e( 'Currency Switcher Position:', 'memberpress' ); ?></label>
						</th>
						<td>
							<select name="mpmc_switcher_position" id="mpmc_switcher_position">
								<option value="" <?php selected( $mpmc_options['mpmc_switcher_position'], 0 ); ?>><?php _e( 'Select', 'memberpress' ); ?></option>
								<option value="before_form" <?php selected( $mpmc_options['mpmc_switcher_position'], 'before_form' ); ?>><?php _e( 'Before Form', 'memberpress' ); ?></option>
								<option value="before_name" <?php selected( $mpmc_options['mpmc_switcher_position'], 'before_name' ); ?>><?php _e( 'Before Name', 'memberpress' ); ?></option>
								<option value="after_email" <?php selected( $mpmc_options['mpmc_switcher_position'], 'after_email' ); // Mapped as "Medium" ?>><?php _e( 'After Email', 'memberpress' ); ?></option>
								<option value="after_password" <?php selected( $mpmc_options['mpmc_switcher_position'], 'after_password' ); ?>><?php _e( 'After Password', 'memberpress' ); ?></option>
								<option value="before_coupon" <?php selected( $mpmc_options['mpmc_switcher_position'], 'before_coupon' ); // Mapped as "Medium" ?>><?php _e( 'Before Coupon', 'memberpress' ); ?></option>
								<option value="before_submit" <?php selected( $mpmc_options['mpmc_switcher_position'], 'before_submit' ); ?>><?php _e( 'Before Submit', 'memberpress' ); ?></option>
							</select>
						</td>
					</tr> 
