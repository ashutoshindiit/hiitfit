...
<div id="mepr-products-form" class="inside">
  ...
  <p>
	...
	if ( 'manual' == $mpmc_options['mpmc_service_provider'] ) {
		...
		foreach ( $product_currencies as $currency ) {
			$currency_attr = MpmcHelper::get_currency_by_code( $currency );
			$price         = ! empty( get_post_meta( $product->ID, MeprProduct::$price_str . '_' . $currency, true ) ) ? get_post_meta( $product->ID, MeprProduct::$price_str . '_' . $currency, true ) : 0;

			?>
			<p>
			<?php /* CodeFusion80 start*/; ?>
				<strong><?php printf( __( 'Price (%1$s):', 'memberpress' ), $currency_attr['symbol'] ); ?></strong>
			<?php /* CodeFusion80 end*/; ?>
			</p>
			<p>
				<input name="<?php echo MeprProduct::$price_str . '_' . $currency; ?>" id="<?php echo MeprProduct::$price_str . '_' . $currency; ?>" type="text" value="<?php echo MeprUtils::format_float( $price ); ?>" />
			</p>
			<?php
		}
	}
	...
	<?php /* CodeFusion80 start*/; ?>
<p>
	<strong><?php _e( 'Force Currency:', 'memberpress' ); ?></strong>
  </p>
  <p>
  <?php
		$product_currencies = $mpmc_options['mpmc_currencies'];
		$force_currency	= ! empty( get_post_meta( $product->ID, MeprProduct::$price_str . '_force_currency' , true ) ) ? get_post_meta( $product->ID, MeprProduct::$price_str . '_force_currency', true ) : '';
		?>
		
		<select name="<?php echo MeprProduct::$price_str . '_force_currency'; ?>"
				id="<?php echo MeprProduct::$price_str . '_force_currency'; ?>">
				<option value=""<?php selected( $force_currency, '' ); ?>><?php _e( 'None', 'memberpress' ); ?></option>
		  
		<?php
		foreach ( $product_currencies as $currency ) {
			$currency_attr = MpmcHelper::get_currency_by_code( $currency );
			?>
			<option value="<?php echo $currency; ?>"<?php selected( $force_currency, $currency ); ?>><?php echo $currency_attr['symbol']; ?></option>
			<?php
		}
	?>
		</select>
...
<?php /* CodeFusion80 end*/; ?>
  ...
  <div id="mepr-recurring-options" class="mepr-hidden">
	...
	<p>
		<?php
		echo MeprProductsHelper::preset_period_dropdown(
	...
	<div class="mepr-product-trial-box">
	  ...
	  <div class="mepr-product-trial-hidden mepr-sub-box">
		...
		if ( 'manual' == $mpmc_options['mpmc_service_provider'] ) {
			...
			foreach ( $product_currencies as $currency ) {
				$currency_attr = MpmcHelper::get_currency_by_code( $currency );
				$price         = ! empty( get_post_meta( $product->ID, MeprProduct::$trial_amount_str . '_' . $currency, true ) ) ? get_post_meta( $product->ID, MeprProduct::$trial_amount_str . '_' . $currency, true ) : 0;
				?>
			<p>
			  <strong><?php printf( __( 'Price (%1$s %2$s):', 'memberpress' ), $currency, $currency_attr['symbol'] ); ?></strong>
			</p>
			<p>
			  <input name="<?php echo MeprProduct::$trial_amount_str . '_' . $currency; ?>" id="<?php echo MeprProduct::$trial_amount_str . '_' . $currency; ?>" type="text" value="<?php echo MeprUtils::format_float( $price ); ?>" />
			</p>
				<?php
			}
		}
		...
		<p>
		  <input type="checkbox" name="<?php echo MeprProduct::$trial_once_str; ?>" id="<?php echo MeprProduct::$trial_once_str; ?>" <?php checked( $product->trial_once ); ?> /> <label for="<?php echo MeprProduct::$trial_once_str; ?>"><?php _e( 'Allow Only One Trial', 'memberpress' ); ?>        			<?php
			MeprAppHelper::info_tooltip(
