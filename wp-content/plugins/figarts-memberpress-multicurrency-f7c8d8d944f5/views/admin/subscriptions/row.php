		...
		foreach ( $columns as $column_name => $column_display_name ) {
			  ...
			  <a href="<?php echo $view_url; ?>" title="<?php _e( 'View related transactions', 'memberpress' ); ?>"><?php _e( 'View Txns', 'memberpress' ); ?></a> |
					...
					if ( $sub->can( 'suspend-subscriptions' ) ) :
						if ( $sub->status == MeprSubscription::$active_str ) {
							$hide_suspend = '';
							$hide_resume  = ' mepr-hidden'; } elseif ( $sub->status == MeprSubscription::$suspended_str ) {
		  ...
		  <td <?php echo $attributes; ?>><?php echo $product_link; ?></td>
				...
				case 'col_txn_product_meta':
					// Begins MemberPress MultiCurrency
					$currency_code = '';

					if ( trim( $rec->sub_type ) == 'transaction' || 'memberpress-lifetimes' == $_GET['page'] ) {
						$currency_code = MpmcHelper::get_subtxn_curr_by_id( $rec->id, 'transaction' );
					} else {
						$currency_code = MpmcHelper::get_subtxn_curr_by_id( $rec->id, 'subscription' );
					}
					...
					// Ends MemberPress MultiCurrency
					?>
		  <td <?php echo $attributes; ?>>
					<?php
					if ( $table->lifetime ) {
					...
					} elseif ( $rec->status == MeprSubscription::$pending_str && empty( $currency_code ) ) {
						$prd = new MeprProduct( $rec->product_id );
