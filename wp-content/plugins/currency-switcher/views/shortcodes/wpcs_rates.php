...
<?php if (!empty($currencies)): ?>
        <select class="wpcs_rates_current_currency" data-precision="<?php echo $precision ?>" data-exclude="<?php echo $exclude_string ?>">
    <?php
    if (!empty($currencies)) {
        foreach ($currencies as $key => $c) {
            ...
            ?>
                    <option <?php selected($current_currency, $key) ?> value="<?php echo $key ?>"><?php printf(__('1 %s is', 'currency-switcher'), $c['name']) ?></option>
                    <?php
                }
            }
            ?>
        ...
        <ul class="wpcs_currency_rates">            
            <?php foreach ($currencies as $key => $c) : ?>
                ...
                <li>
                <?php if (!empty($c['flag'])): ?>
                        <img src="<?php echo $c['flag'] ?>" width="25" alt="<?php echo $c['name'] ?>" />&nbsp;
                <?php endif; ?>
