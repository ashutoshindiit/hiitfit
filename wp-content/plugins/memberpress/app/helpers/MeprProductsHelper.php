  ...
  public static function display_invoice( $product, $coupon_code = false, &$payment_required = true ) {
    ...
    if($product->is_one_time_payment()) {
        ...
        if($product->register_price_action == 'custom') {
          ...
          if ( !empty($force_currency) && MpmcHelper::$base_currency !== $force_currency ) {
            if(!$mepr_options->currency_symbol_after) {
              echo $mepr_options->currency_symbol . MeprAppHelper::format_price_string($tmp_txn, $tmp_txn->amount, false, $coupon_code, true, $payment_required);
            }
            else {
              echo MeprAppHelper::format_price_string($tmp_txn, $tmp_txn->amount, false, $coupon_code, true, $payment_required) . $mepr_options->currency_symbol;
            }
          }
          else {
      ...
      if($product->register_price_action == 'custom' && empty($coupon_code) && !$tmp_sub->prorated_trial) {
        ...
        if ( !empty($force_currency) && MpmcHelper::$base_currency !== $force_currency ) {
          if(!$mepr_options->currency_symbol_after) {
            echo $mepr_options->currency_symbol . MeprAppHelper::format_price_string($tmp_sub, $tmp_sub->price, false, $coupon_code, true, $payment_required);
          }
          else {
            echo MeprAppHelper::format_price_string($tmp_sub, $tmp_sub->price, false, $coupon_code, true, $payment_required) . $mepr_options->currency_symbol;
          }
        }
        else {
